...
# ---------- 데이터프레임에 map 적용 ----------
# titanic 에서 age,fare 선택하여 df만들기
...
# 각 열에 calculate_staus 함수 적용
result_df=df.apply(calculate_status,axis=0)
print(result_df)
print()
...
result_df3=df.apply(lambda row,multi: pd.Series([(row.max() - row.min())*multi,row.mean()],index=['차이','평균']),axis=0,multi=2)

print(result_df3)
print()

# 평균 값이 30을 초과하는 열만 필터링
...
# df의 각 행의 평균이 50을 초과하면 'Yes' 아니면 'No'인 컬럼 'High' 만들기 (apply, 람다식)
