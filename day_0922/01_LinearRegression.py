...
# (a 로 정리한) 오차제곱은 아래로 볼록한 포물선
# a가 몇일때 이 포물선이 최솟값을 가질까?
# 이 이차함수를 a에 대하여 미분한 값이 0이 되는 그때의 a값이다.
# b 에 대해서도 같은 과정을 거친 뒤, 두 식을 연립 방정식해서 a,b 값 도출
...
print(lr_linear.score(train_input, train_target))
print(lr_linear.score(test_input, test_target))
